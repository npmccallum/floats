...
impl PartialEq for f16 {
    ...
    fn eq(&self, other: &Self) -> bool {
        ...
        // exponent = 0x1F (all 1s) AND mantissa != 0
...
impl f16 {
    ...
    /// Creates an `f16` from its representation as a `u16`.
    #[inline]
    pub const fn from_bits(bits: u16) -> Self {
        Self(bits)
    }

    /// Creates an `f16` from its representation as a byte array in big endian.
    #[inline]
    pub const fn from_be_bytes(bytes: [u8; 2]) -> Self {
        Self(u16::from_be_bytes(bytes))
    }

    /// Creates an `f16` from its representation as a byte array in little endian.
    #[inline]
    pub const fn from_le_bytes(bytes: [u8; 2]) -> Self {
        Self(u16::from_le_bytes(bytes))
    }

    /// Creates an `f16` from its representation as a byte array in native endian.
    #[inline]
    pub const fn from_ne_bytes(bytes: [u8; 2]) -> Self {
        Self(u16::from_ne_bytes(bytes))
    }

    /// Returns the raw representation of this `f16` as a `u16`.
    #[inline]
    pub const fn to_bits(self) -> u16 {
        self.0
    }

    /// Returns the memory representation of this `f16` as a byte array in big endian.
    #[inline]
    pub const fn to_be_bytes(self) -> [u8; 2] {
        self.0.to_be_bytes()
    }

    /// Returns the memory representation of this `f16` as a byte array in little endian.
    #[inline]
    pub const fn to_le_bytes(self) -> [u8; 2] {
        self.0.to_le_bytes()
    }

    /// Returns the memory representation of this `f16` as a byte array in native endian.
    #[inline]
